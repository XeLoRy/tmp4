---
title: Vie Locale
icon: /images/pictos/vie-locale.png
description: Renforcer la vie locale et le lien social entre les habitants.
ordre: 3
engagements:
  - Construire un centre de services de proximité au Petit-Bornand
  - Valoriser les associations et encourager les initiatives locales
  - Création d'une salle de motricité et d'un réfectoire à l'école d'Entremont
  - Améliorer la mobilité douce et sécuriser les espaces publics
---
### 1. Construire un centre de services de proximité au Petit-Bornand avec :

* Une maison médicale (médecin, cabinet d'infirmiers(ères), professions paramédicales et de bien-être…)
* Une salle polyvalente pour les associations, les réunions publiques et les fêtes privées (anniversaires, baptêmes, cousinades…)
* Des logements adaptés pour personnes âgées en perte d'autonomie pour qu'elles puissent rester au village
* Un dispositif d'hébergements d'urgence

### 2. Valoriser les associations et encourager les initiatives locales

* Créer un service vie associative pour apporter un réel soutien aux associations par tous les moyens dont dispose la commune (humains, financiers, locaux à disposition, communication, planning des rendez-vous associatifs…)
* Organiser une soirée pour remercier et valoriser les associations
* Harmoniser la gestion de la cantine et du périscolaire entre les écoles de Petit-Bornand et d'Entremont

### 3. Créer une salle de motricité et d'un réfectoire à l'école d'Entremont

* Associer le nouveau réfectoire et la nouvelle salle de motricité dans un même projet, en collaboration avec la CCFG
* Anticiper la possibilité d'une future cuisine sur place
* Associer l'APE et l'équipe enseignante au projet

### 4. Améliorer la mobilité douce et sécuriser les espaces publics

* Etudier la possibilité de mettre en place un passage à mobilité douce reliant les deux villages en concertation avec les propriétaires et les habitants
* Encourager le covoiturage entre les habitants
* Réhabiliter et sécuriser les abribus, créer des parkings à vélos à proximité
* Créer/réfectionner les liaisons douces entre les hameaux
* Sécuriser les voies piétonnes sur la RD12 en concertation avec nos partenaires institutionnels
* Rétablir l’éclairage public à des horaires adaptés pour la sécurité

## Notre objectif

**Renforcer la vie locale et le lien social entre les habitants.**\
**Une commune où il fait bon vivre, ensemble !**
